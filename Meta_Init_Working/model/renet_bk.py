...
#            nn.init.orthogonal_(parameters[each_key])
        #Adding forgetgate bias as 1, i.e. update gate is 1 so we are learning all info from the backward.
        #iF its -1 then it will take all the input from current timestamp
        # https://github.com/abhishekaich27/Navsynth/blob/master/models.py
...
#             RandomCrop(size=(32,32), padding=4),
        ...
        self.model = nn.Sequential(OrderedDict([
            ('renet1', ReNet(2 * 2 * 3, 160, kernel_size=(2, 2))),
            ('dropout1', nn.Dropout(p=0.3)),
            ('renet2', ReNet(2 * 2 * 320, 160, kernel_size=(2, 2))),
            ('dropout2', nn.Dropout(p=0.3)),
            ('renet3', ReNet(2 * 2 * 320, 160, kernel_size=(2, 2))),
...
#            nn.Dropout(p=0.3),
            ...
            ('flatten1',nn.Flatten()),
#            nn.Linear(320 * 8 * 8, 4096),
            ('fc1', nn.Linear(320 * 4 * 4, 4096)),
#            nn.Linear(320 * 1 * 1, 4096),
    ...
    def configure_optimizers(self):
        if self.optimizer == 'SGD':
            ...
            return optim.Adam(self.parameters(), lr=self.lr, weight_decay=5e-4)
...
def printnorm(self, input, output):
    ...
    a_1.append(output.data)
    return output
    
if __name__ == '__main__':
    renet = nn.Sequential(OrderedDict([
       ('renet1', ReNet(2 * 2 * 3, 128, kernel_size=(2, 2))), 
       ('renet2', ReNet(2 * 2 * 256, 128, kernel_size=(2, 2))),
       ('renet3', ReNet(2 * 2 * 256, 128, kernel_size=(2, 2))),
       ('flatten', nn.Flatten()),
       ('fc1', nn.Linear(256 * 4 * 4, 4096)),
       ('relu1', nn.ReLU()),
       ('fc2', nn.Linear(4096, 10))])
    )
...
#    activation_1 = renet.renet1.register_forward_hook(printnorm)
#    activation_2 = renet.renet2.register_forward_hook(printnorm)
#    print("Activation 1 shape:",type(activation_1))
#    print("Activation 2 shape:",type(activation_2))
#    print("Activation map is:", a_1)
    renet_renet1 = nn.Sequential(*list(renet.children())[:-6])
    ...
    print("RENET 1 activation is:",avg_acts1.shape)
    x = renet(a)
    print(x.size())
