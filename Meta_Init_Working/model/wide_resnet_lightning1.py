...
class PL_Wide_Resnet(LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--dropout", default=0.3, type=int, help="dropout_rate")
    ...
    def configure_optimizers(self):
        if self.optimizer == 'SGD':
            ...
            optimizer = optim.SGD(self.parameters(), lr=self.lr, momentum=self.momentum, weight_decay=5e-4)
            #lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer)
            lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,"min")
#            scheduler = {'scheduler': lr_scheduler, 'interval': 'step','monitor':'val_checkpoint_on','min_lr': 1e-8}
            scheduler = {'scheduler': lr_scheduler,"reduce_on_plateau": True, 'interval': 'step','monitor':'val_checkpoint_on','min_lr': 1e-8}
